...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='hivitb_224', help='yaml configure file name')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "/root/anaconda3/envs/ssttrack/bin/python3.8 lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "/root/anaconda3/envs/ssttrack/bin/python3.8 -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
