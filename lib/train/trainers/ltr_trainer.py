...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        """训练一个epoch的所有loader, loader.__len__()的长度 = SAMPLE_PER_EPOCH/batch_size = 50/8 = 6"""
        """ Do a cycle of training or validation."""
        self.actor.train(loader.training)  # 切换train模式
        torch.set_grad_enabled(loader.training)
        self._init_timing()  # 计时

        # 读取1个epoch的1个个loader数据, 并进行默认的处理
        for i, data in enumerate(loader, 1):
            ...
            if not self.use_amp:
                ...
                # 前向传递
                # 调用actor在lib/train/actors/aqatrack.py, 包含forward_pass()和compute_losses()函数
                # 其forward_pass()调用lib/models/aqatrack/aqatrack.py中AQATrack类的forward()函数
            ...
            # 打印statistics, 每个epoch数据加载完才打印, i.e., loader.__len__()=SAMPLE_PER_EPOCH/batch_size=50/8=6
    ...
    def _print_stats(self, i, loader, batch_size):
        ...
        if i % self.settings.print_interval == 0 or i == loader.__len__():
            ...
            # 打印最后一个loader的状态
            print_str = '[%s: %d, %d /loader_len: %d] ' % (loader.name, self.epoch, i, loader.__len__())
