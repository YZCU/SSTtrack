...
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, HSI
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        ...
        assert name in ["hsi_train", "hsi_val"]

        if name == "hsi_train":
            datasets.append(HSI(settings.env.hsi_dir, split=None, image_loader=image_loader))

        if name == "hsi_val":
            datasets.append(HSI(settings.env.hsi_val_dir, split=None, image_loader=image_loader))
...
def build_dataloaders(cfg, settings):
    ...
    transform_train = tfm.Transform(tfm.ToTensorAndJitter(0.2), tfm.RandomHorizontalFlip_Norm(probability=0.5),
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        for name, param in net.named_parameters():
            if any(t_param in name for t_param in ['.cs_', '.a_t.', '.a2_t.', 'stmf']):
                param.requires_grad = True
            else:
                param.requires_grad = False
        param_dicts = [{"params": [p for n, p in net.named_parameters() if "backbone" not in n and p.requires_grad]},
