...
def split_channel(feat_channel, order):
    ...
    o_fc_imp = [[None for j in range(b)] for i in range(5)]
    obi = order[0]
    ordb = order[1]
    for i in range(5):
        stb = ordb[0, i * 3:i * 3 + 3]
        gg = feat_channel[None, 0, stb, :, :]
        o_fc_imp[i][0] = obi[0, i * 3:i * 3 + 3].detach().mean().item()
        for k in range(1, b):
            stb_one = ordb[k, i * 3:i * 3 + 3]
            gg = torch.cat((gg, feat_channel[None, k, stb_one, :, :]), dim=0)
            o_fc_imp[i][k] = obi[k, i * 3:i * 3 + 3].detach().mean().item()
        bres.append(gg)
    return bres, o_fc_imp
